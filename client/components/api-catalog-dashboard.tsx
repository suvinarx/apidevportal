    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md rounded-2xl shadow-xl border border-red-200 bg-white animate-fade-in">
        <DialogHeader className="flex flex-col items-center gap-2 text-center">
          ...
          <DialogTitle className="text-xl font-semibold text-red-600">File Too Large</DialogTitle>
        ...
        <DialogFooter className="mt-4">
          <Button
            variant="outline"
            onClick={onClose}
            className="w-full text-red-600 border-red-200 hover:bg-red-50 transition bg-transparent"
          >
            Close
          </Button>
        </DialogFooter>
...
const getCategoryColor = (category?: string | { _id: string; name: string }) => {
  ...
  if (!categoryName) return "bg-amber-100 text-amber-800 border-amber-200 hover:bg-amber-50 transition-colors"
...
export default function APICatalogDashboard({ role }: { role: "admin" | "user" }) {
  ...
  const toggleFilter = (arr: string[], value: string, setter: any) => {
    setter(arr.includes(value) ? arr.filter((id) => id !== value) : [...arr, value])
  }
  ...
  // Form states - removed tags from initial state
  ...
  const filteredCatalogs = catalogs.filter((catalog) => {
    ...
    // Removed tag search functionality
  ...
  const handleSubmit = async (isEdit: boolean) => {
    try {
      ...
      setFormData({
        ...
        status: "active",
        accessRoles: ["admin"],
        tags: [], // Keep empty for API compatibility
      })
      setOpenApiFile(null)
  ...
  // Removed tags input handling functions
  ...
  return (
    <>
      <DashboardHeader catalogs={catalogs} handleViewDocumentation={handleViewDocumentation} />
      ...
      <div className="max-w-[1550px] mx-auto px-4 py-8 ">
        ...
        <div className="flex">
          ...
          <div className="px-6 py-6 bg-white">
            <h1 className="text-4xl font-bold text-gray-900">Explore Our API</h1>
            <p className="mt-2 text-gray-600 text-base">
              Browse Girlscouts APIs to find the right solution for you. Use the filter options to refine your search
              further..
            </p>
          </div>
        </div>
        <div className="flex min-h-screen ">
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          {selectedBusinessTypes.length > 0 && (
                            <span className="text-xs text-gray-500">({selectedBusinessTypes.length})</span>
                          )}
                              ...
                              {businessTypes.map((type) => (
                                ...
                                >
                                  <div className="flex items-center">
                                    <input
                                      type="checkbox"
                                      checked={selectedBusinessTypes.includes(type._id)}
                                      onChange={() =>
                                        toggleFilter(selectedBusinessTypes, type._id, setSelectedBusinessTypes)
                                      }
                            ...
                            return (
                              ...
                              >
                                ...
                                <X
                                  ...
                                  onClick={() => setSelectedRegions(selectedRegions.filter((id) => id !== rId))}
                                />
                              </span>
                            )
                          })}
                            ...
                            return (
                              ...
                              >
                                ...
                                <X
                                  className="w-3 h-3 cursor-pointer"
                                  onClick={() =>
                                    setSelectedBusinessTypes(selectedBusinessTypes.filter((id) => id !== bId))
                                  }
                                />
                              </span>
                            )
                          ...
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2">
                                <Badge
                                  className={`${getCategoryColor(catalog.category)} text-xs font-medium px-2.5 py-1`}
                                >
                                ...
                                <Badge className={`${getStatusColor(catalog.status)} text-xs font-medium px-2.5 py-1`}>
                                  {catalog.status}
                                </Badge>
                              </div>
                            </div>
          ...
          {/* Add/Edit API Modal */}
          <FileSizeErrorModal isOpen={isFileSizeErrorModalOpen} onClose={() => setIsFileSizeErrorModalOpen(false)} />
          <Dialog
          ...
          >
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="grid gap-6 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button
                          variant="outline"
                          className="w-full justify-start text-left font-normal truncate bg-transparent"
                        >
                          {formData.businessTypes?.length
                            ? businessTypes
                              .filter((b) => formData.businessTypes?.includes(b._id))
                              .map((b) => b.name)
                              .join(", ")
                        ...
                        {businessTypes.map((type) => (
                          <div
                            ...
                            onClick={() => {
                              setFormData((prev) => ({
                                ...prev,
                                businessTypes: prev.businessTypes?.includes(type._id)
                                  ? prev.businessTypes.filter((id) => id !== type._id)
                ...
                {/* Removed tags input section */}
                <div className="space-y-2">
                  <Label htmlFor="openapi">Import OpenAPI File (optional)</Label>
                  <div
                    className={`      relative flex flex-col items-center justify-center border-2 border-dashed       ${openApiFileError ? "border-red-400 bg-red-50" : "border-emerald-200 bg-emerald-50/60"
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          setOpenApiFileError(
                            "File parsing failed: " + (err instanceof Error ? err.message : "Invalid format"),
                          )
                          setFormData({ ...formData, openapiSpec: undefined })
                        }
                      }}
